...
# Get all "non-branch" commits reachable from main
MAIN_COMMITS=$(git rev-list --first-parent origin/main)

# Only use tags that are on one of those commits and
# have the right kind of name.
if [[ ! "$MAIN_COMMITS" =~ "${GITHUB_COMMIT_ID}" ]]
then
  echo "Tag is not reachable from main, we will skip this tag"
  exit 0
...
then
  ...
  echo "Tag looks good: right format and reachable from main"
fi
